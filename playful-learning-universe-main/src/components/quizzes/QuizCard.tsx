...
const QuizCard = ({ 
  ...
  const getCategoryColor = () => {
    switch(category.toLowerCase()) {
      case 'science': return 'bg-kid-green text-white';
      case 'math': return 'bg-kid-blue text-white';
      case 'spelling': return 'bg-kid-purple text-white';
      case 'geography': return 'bg-kid-teal text-white';
      default: return 'bg-kid-yellow text-kid-blue';
    }
  };
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between mb-4">
        <span className={`kid-badge ${getCategoryColor()}`}>
          {category}
        </span>
      ...
      {imageUrl && (
        <div className="h-40 overflow-hidden rounded-xl mb-4">
          <img src={imageUrl} alt="Quiz question" className="w-full h-full object-cover" />
        </div>
      )}
        ...
        {options.map((option) => (
          ...
          >
            ...
            <AnimatePresence>
              ...
              {showFeedback && option.id === selectedOptionId && option.id !== correctOptionId && (
                <motion.div 
                  className="absolute right-4 top-1/2 transform -translate-y-1/2"
                  initial={{ scale: 0, opacity: 0 }}
                  animate={{ scale: 1, opacity: 1 }}
